%%
%% @doc 基于content store的group database模块.
%%
%%
-module(dbcs_group).
-compile(export_all).
...
%%	Advance = [dbcs_base:term2db(K,V)||{K,V}<- Group,base_property(K)=:=false],
	case [is_atom(Id)] of
		[true]->
			{content,list_to_atom(?Table), Id, "group", null,null,null,null,?Author,null,null,null,null,null,Group};
		_->
			{}
	end.
...
%%	case db_ecc:insert_data(?DBName,?Table,group_to_db(Group)) of
	case db_ecc:insert_data(?DBName,?Table,Group) of
		{ok,Ret}->
			{ok,db_to_group(Ret)};
		{error,Err}->
			{error,Err};
...
update_group(Group)->
	...
	case db_ecc:update_data(?DBName,?Table,"id=" ++ atom_to_list(Id),ND) of
		{ok,Ret}->
			{ok,db_to_group(Ret)};
		{error,Err}->
			{error,Err};
