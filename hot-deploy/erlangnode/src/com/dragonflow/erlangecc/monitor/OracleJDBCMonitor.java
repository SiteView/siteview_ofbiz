	...
	public Map<String, Object> update() {
		...
		try {
			...
			label: for (int i = 0; i < counter_size && counter_it.hasNext(); i++) {
				...
				if(lablename.indexOf("Tablespace")>=0)
				{
					if(queryMap.containsKey(counter))
					{
						states[i]=queryMap.get(counter).toString();
...
//			results.put(COUNTERS_VALUE, counter_list);
	...
	private HashMap<String, Object> queryValues(Connection conn)
	{
		...
		//10 tablespace
		strsql="SELECT   a.tablespace_name   tablespacename,b.total  totaltablespace,a.free   freetablespace, "+
        "ROUND((b.total-a.free)/b.total,4)   usedpercent "+
        " FROM     (SELECT   tablespace_name,SUM(bytes)/1024/1024   free   FROM   DBA_FREE_SPACE "+
        " GROUP   BY   tablespace_name   )   a,   "+
        "(SELECT   tablespace_name,SUM(bytes)/1024/1024   total   FROM   DBA_DATA_FILES "+
        "GROUP   BY   tablespace_name)   b "+
        "WHERE   a.tablespace_name=b.tablespace_name   order by b.tablespace_name";
		statement = conn.prepareStatement(strsql);
		statement.setQueryTimeout(this.query_timeout);
		results = statement.executeQuery();
		while (results.next()) {
		  String tablespacename= results.getString("tablespacename");
		  queryValues.put(tablespacename+"_"+"totaltablespace", results.getDouble("totaltablespace"));
		  queryValues.put(tablespacename+"_"+"freetablespace", results.getDouble("freetablespace"));
		  queryValues.put(tablespacename+"_"+"usedpercent", results.getDouble("usedpercent"));
		}
		//11
		String total_users="n/a";
		String active_users="n/a";
		strsql="select count(username) total_users,count(distinct username) active_users from v$session";
		statement = conn.prepareStatement(strsql);
		statement.setQueryTimeout(this.query_timeout);
		results = statement.executeQuery();
		while (results.next()) {
			...
			queryValues.put("totalusers", total_users);
			queryValues.put("activeusers", active_users);
...
System.out.println("------get database connection url: " + jdbcUrl);
    ...
    private String buildXML(Map<String, Object> map, List counterSysResult, List instanceSysResult, List instanceSesResult, List tableSpaceQuery, List tableQueryArray[],Connection conn)
    {
...
//        }
        ...
        if (map.size()>0)
        {
        	...
        	map.put("totalusers", "oracle Running/total users");
        	map.put("activeusers", "oracle Running/active users");
    		...
    		ResultSet results = null;
    		String strsql = "";
    		 strsql="SELECT  distinct tablespace_name  FROM   DBA_DATA_FILES  order by tablespace_name ";
             try {
				statement = conn.prepareStatement(strsql);
				statement.setQueryTimeout(this.query_timeout);
	            results = statement.executeQuery();
	            ...
	            while (results.next()) {
                        String tablespace_name=results.getString("tablespace_name");
                        map.put(tablespace_name, "Tablespace"+"/"+tablespace_name);
                        map.put(tablespace_name+"_"+"totaltablespace","Tablespace"+"/"+tablespace_name+"/"+ "totaltablespace");
                        map.put(tablespace_name+"_"+"freetablespace","Tablespace"+"/"+ tablespace_name+"/"+"freetablespace");
                        map.put(tablespace_name+"_"+"usedpercent","Tablespace"+"/"+ tablespace_name+"/"+"usedpercent");
	             }
			} catch (SQLException e) {
	...
	Map<String, Object> getBrowseData() {
		...
		try {
		    ...
		    buildXML(resp, counterNameList, instanceNameList, sidValueList, freeTableSpaceList, tableQueryArray,conn);
